    ...
    if ($phone) {
        ...
        if ($ifNotExist) {
            if ($counter === 0) {
                ...
                (vc_phone,vc_fio,sex_id,source_id,region_id,town_id,tx_location,vc_email,bn_hash,created_at)
                VALUES
                ('$phone','$name',$sex,'$source','$region','$city','$location','$email',UNHEX('$hash'),now())";
            } else {
                $query .= ",('$phone','$name',$sex,'$source','$region','$city','$location','$email',UNHEX('$hash'),now())";
            }
            $counter++;
            if ($counter === 300) {
                $result = mysqli_query($link, $query);
