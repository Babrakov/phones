...
//        print_r($town);
        ...
        if (trim($arr[10]) === '#N/A') {
            ...
            $city = getTownId(trim($arr[10]), $link);
        }
...
//continue;
    if (!$phone || (is_null($city) && $arr[10]) || (is_null($region) && $arr[7])) {
...
{
    $region = str_replace('Российская Федерация','не указан',$region);
    $region = str_replace('Ханты - Мансийский АО - Югра','Ханты-Мансийский',$region);
