...
//    $arr = explode(";",$str);
    ...
    $region = trim(preg_replace('/(край|область|обл\.|\)|АО)/','',$arr[7]));
...
function getRegionId($str,$link)
{
    $query = "SELECT id FROM regions WHERE vc_name LIKE '$str%' OR vc_sname LIKE '$str%'";
    $result = mysqli_query($link, $query);
    $num = mysqli_num_rows($result);
//    echo $query;
    if ($num > 0) {
